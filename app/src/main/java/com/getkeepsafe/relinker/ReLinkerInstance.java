...
public class ReLinkerInstance {
    ...
    public boolean loadLibraryBlocking(final Context context,
                            final String library,
                            final String version                            ) {
        if (context == null) {
            throw new IllegalArgumentException("Given context is null");
        ...
        log("Beginning load of %s...", library);
        try {
            return loadLibraryInternalWithResult(context, library, version);
        } catch (UnsatisfiedLinkError e) {
            return false;
        } catch (MissingLibraryException e) {
            return false;
    ...
    private boolean loadLibraryInternalWithResult(final Context context,
                                     final String library,
                                     final String version) {
        if (loadedLibraries.contains(library) && !force) {
            log("%s already loaded previously!", library);
        ...
        try {
            libraryLoader.loadPath(workaroundFile.getAbsolutePath()); // may throw exception
            loadedLibraries.add(library);
            log("%s (%s) was re-linked!", library, version);
            return true;
        } catch (Exception e) {
