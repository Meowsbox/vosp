...
public class SpectrumDialog extends DialogFragment implements SpectrumPalette.OnColorSelectedListener {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (args != null && args.containsKey(KEY_TITLE)) {
            ...
            mTitle = getText(R.string.default_dialog_title);
        }
        ...
        if (mSelectedColor == -1) // retrieve the static argument if a value has not already been set previously
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (mThemeResId != 0) {
            builder = new AlertDialog.Builder(getActivity(), mThemeResId);
        } else {
...
//        mListener = null;
    ...
    public void setSelectedColorIndex(int index) {
        if (index < 0 || index >= mColors.length)
            throw new IndexOutOfBoundsException("Index must be within the array of available colors");
        mSelectedColor = mColors[0];
    }

    /**
     * Call to update the selected color of a dialog for reuse.
     *
     * @param color A color from the defined palette, otherwise behavior undefined.
     */
    public void updateSelectedColor(@ColorInt int color) {
        mSelectedColor = color;
        Dialog dialog = getDialog();
        if (dialog != null) {
            SpectrumPalette palette = (SpectrumPalette) getDialog().findViewById(R.id.palette);
            if (palette != null) {
                palette.setSelectedColor(mSelectedColor);
                palette.invalidate();
            }
